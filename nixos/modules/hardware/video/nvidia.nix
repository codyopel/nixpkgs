...
let
  ...
  nvidiaForKernel =
    ...
    if elem "nvidiaLegacy340" drivers then
...

{
  ...
  config = mkIf enabled {
    ...
    boot.extraModulePackages = [ nvidia_-drivers ];

    # nvidia-uvm is required by CUDA applications. (x86_64 only)
    boot.kernelModules = optionals nvidia-drivers.cudaUVM [
      "nvidia-uvm"
    ];
    ...
    services.udev.extraRules = ''
      KERNEL=="nvidia_uvm", \
        RUN+="${pkgs.stdenv.shell} \
        -c 'mknod -m 666 /dev/nvidia-uvm c $(
    ...
    environment.etc."OpenCL/vendors/nvidia.icd".source =
      "${nvidia_x11}/lib/vendors/nvidia.icd"
    ;
