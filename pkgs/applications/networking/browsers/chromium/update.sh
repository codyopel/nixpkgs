...
sha_lookup() {
    ...
    for ver_sha in $ver_sha_table ; do
        if [ "x${ver_sha%:*}" = "x$version" ] ; then
            echo "${ver_sha##*:}"
            return 0
        fi
    done
...
get_newest_ver() {
    ...
    if oldest="$(echo "$versions" | sort -V 2> /dev/null | tail -n1)" ; then
        echo "$oldest"
    else
...
get_prev_sha256() {
    ...
    for version in $(fetch_filtered_history "$channel") ; do
...
get_channel_exprs() {
    for chline in $1 ; do
        ...
        if sha256="$(sha_lookup "$version")" ; then
            echo " yes: $sha256" >&2
        else
            echo " no." >&2
        ...
        echo "    sha256 = \"$main\";"
        if [ "x${deb#*[a-z0-9].[a-z0-9]}" != "x$deb" ] ; then
